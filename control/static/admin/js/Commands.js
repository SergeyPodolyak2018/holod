...
function start_stop_mex(name,command){        
  let url_string ='/io_command/?'+
                  'name'+'='+name+'&'+
                  'command'+'='+command;        
  objectMenuManager.hide();
  let bufer= {name:name};                                                     //window lockation
  get_data_to_server(url_string,null,bufer);
}

//Фукция закытия меню датчиков
  function open_datchiki(name){
                ...
                if (k.charAt(0) =='d') {
                  if (dat_cont[k].alarm == 0){
                    if (dat_cont[k].status == 1) {
                      ...
                      $(indikator_datchiki).css('box-shadow','0 0 25px #E5E5E6');                     //серый
                      $(indikator_datchiki).css('-webkit-box-shadow','0 0 25px #E5E5E6');             //серый
